...
<div class="my-3">
	...
	//カレンダーを表示するHTML
	print '<table class="table table-bordered"><tr><td colspan=7>';
	print "<center>";
	if ($data['prev']) {
	...
	function check($i, $w, $year, $month, $day, $entryDates, $BcBaser, $blogContent) {
		if (in_array(date('Y-m-d', strtotime($year . '-' . $month . '-' . $i)), $entryDates)) {
			if (date('Y-m-d') == date('Y-m-d', strtotime($year . '-' . $month . '-' . $i))) {
				$change = '<td class="bg-success">' . $BcBaser->getLink($i, $BcBaser->getBlogContentsUrl($blogContent['BlogContent']['id']) . 'archives/date/' . $year . '/' .  $month . '/' . $i, null, false) . '</td>';
			} elseif ($w == 0) {
				$change = '<td class="bg-danger">' . $BcBaser->getLink($i, $BcBaser->getBlogContentsUrl($blogContent['BlogContent']['id']) . 'archives/date/' . $year . '/' .  $month . '/' . $i, null, false) . '</td>';
			} elseif ($w == 6) {
				$change = '<td class="bg-danger">' . $BcBaser->getLink($i, $BcBaser->getBlogContentsUrl($blogContent['BlogContent']['id']) . 'archives/date/' . $year . '/' .  $month . '/' . $i, null, false) . '</td>';
			} else {
			...
			if (date('Y-m-d') == date('Y-m-d', strtotime($year . '-' . $month . '-' . $i))) {
				$change = '<td class="bg-success">' . $i . '</td>';
			} else {
				$change = '<td>' . $i . '</td>';
			}
		}
